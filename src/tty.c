...
static code_t modify_code(code_t code) {
    ...
    // treat ctrl/shift + enter always as KEY_LINEFEED for portability
             ...
             (mods == KEY_MOD_SHIFT || mods == KEY_MOD_ALT ||
              mods == KEY_MOD_CTRL)) {
        code = KEY_LINEFEED;
    }
        ...
        if (final_st) {
            ...
            if (c == '\x02') {  // STX
                break;
                         ...
                         strchr("<=>?;:", c) != NULL)) {
                buf[len++] =
                    (char)c;  // for non-OSC save the terminating character
                break;
            }
        }
        buf[len++] = (char)c;
...
static void signals_install(tty_t* tty) {
    ...
    for (signal_handler_t* sh = sighandlers; sh->signum != 0; sh++) {
        if (sigaction(sh->signum, NULL, &sh->action.previous) ==
            0) {  // get previous
            if (sh->action.previous.sa_handler !=
                SIG_IGN) {  // if not to be ignored
                if (sigaction(sh->signum, &handler, &sh->action.previous) <
                    0) {  // install our handler
                    sh->action.previous.sa_sigaction =
                        NULL;  // do not restore on error
        ...
        if (timeout_ms >= 0) {
            ...
            if (count == 0) {
                if (timeout_ms == 0) {
                    ...
                    DWORD res =
                        WaitForSingleObject(tty->hcon, (DWORD)timeout_ms);
                    switch (res) {
                        case WAIT_OBJECT_0: {
