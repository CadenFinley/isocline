...
static ssize_t char_column_width(const char* s, ssize_t n) {
    if (s == NULL || n <= 0)
        ...
        return (w <= 0
                    ? 1
                    : w);  // windows console seems to use at least one column
#else
        return w;
#endif
    }
...
static ssize_t str_for_each_row(const char* s, ssize_t len, ssize_t termw,
    ...
    for (i = 0; i < len;) {
        ...
        if (next <= 0) {
            debug_msg(
                "str: foreach row: next<=0: len %zd, i %zd, w %zd, buf %s\n",
                len, i, w, s);
            assert(false);
            break;
        }
...
ic_private ssize_t sbuf_append_vprintf(stringbuf_t* sb, const char* fmt,
    ...
    va_copy(args0, args);
    ssize_t needed =
        vsnprintf(sb->buf + sb->count, to_size_t(avail), fmt, args0);
    if (needed > avail) {
        sb->buf[sb->count] = 0;
...
ic_private ssize_t sbuf_for_each_row(stringbuf_t* sbuf, ssize_t termw,
    ...
    if (sbuf == NULL)
        return 0;
    return str_for_each_row(sbuf->buf, sbuf->count, termw, promptw, cpromptw,
                            fun, arg, res);
}
...
ic_private char* sbuf_strdup_from_utf8(stringbuf_t* sbuf) {
    ...
    for (ssize_t i = 0; i < len;) {
        ...
        if (ofs <= 0) {
            ...
            // decode unicode
            ssize_t nread;
            unicode_t uchr = unicode_from_qutf8((const uint8_t*)(sbuf->buf + i),
                                                ofs, &nread);
            uint8_t c;
...
// found).
ic_public long ic_is_token(const char* s, long pos,
...
ic_public long ic_match_token(const char* s, long pos,
                              ic_is_char_class_fun_t* is_token_char,
                              const char* token) {
...
// returns 0.
ic_public long ic_match_any_token(const char* s, long pos,
                                  ic_is_char_class_fun_t* is_token_char,
                                  const char** tokens) {
    long n = ic_is_token(s, pos, is_token_char);
    if (n <= 0 || tokens == NULL)
