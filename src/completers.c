...
static bool token_add_completion_ex(ic_env_t* env, void* closure,
    ...
    return (*wenv->prev_complete)(
        env, wenv->prev_env, replacement, display, help,
...
static bool qword_add_completion_ex(ic_env_t* env, void* closure,
    ...
    return (*wenv->prev_complete)(
        env, wenv->prev_env, sbuf_string(wenv->sbuf), display, help,
        wenv->delete_before_adjust + delete_before, delete_after);
}
...
ic_public void ic_complete_qword_ex(ic_completion_env_t* cenv,
                                    const char* prefix, ic_completer_fun_t* fun,
                                    ic_is_char_class_fun_t* is_word_char,
    ...
    if (quote_chars[0] != 0) {
        // we go forward and count all quotes; if it is uneven, we need to
        // complete quoted.
        ssize_t qpos_open = -1;
        ...
        while (pos < len) {
                    ...
                    1))  // strchr(non_word_char, prefix[pos+1]) != NULL
            {
                pos++;  // skip escape and next char
                       ...
                       strchr(quote_chars, prefix[pos]) != NULL) {
                // open quote
                qpos_open = pos;
                quote = prefix[pos];
                qcount++;
            ...
            (qcount % 2 == 1))    // opening quote found
        {
            ...
            pos = qpos_open +
                  1;  // pos points to the word start just after the quote.
        } else {
            quote = 0;
        }
    }
        ...
        while (wpos < wlen) {
            ...
            if (word[wpos] == escape_char && word[wpos + 1] != 0 &&
                !(*is_word_char)(
                    word + wpos + 1,
                    (long)
                        ofs))  // strchr(non_word_char, word[wpos+1]) != NULL) {
            {
                ic_memmove(word + wpos, word + wpos + 1,
                           wlen - wpos /* including 0 */);
            }
            wpos += ofs;
        }
    }
    ...
    if (os_findfirst(cenv->env->mem, sbuf_string(dir), &d, &entry)) {
        do {
            ...
            if (name != NULL && strcmp(name, ".") != 0 &&
                strcmp(name, "..") != 0 && ic_istarts_with(name, base_prefix)) {
                ...
                if (isdir || match_extension(name, extensions)) {
                    ...
                    ls_colorize(cenv->env->no_lscolors, display, ft, name, NULL,
                                (isdir ? dir_sep : 0));
                    cont = ic_add_completion_ex(cenv, sbuf_string(dir_prefix),
...
ic_public void ic_complete_filename(ic_completion_env_t* cenv,
    ...
    cenv->arg = &fclosure;
    ic_complete_qword_ex(cenv, prefix, &filename_completer,
