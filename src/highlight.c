...
static void pos_adjust(ic_highlight_env_t* henv, ssize_t* ppos, ssize_t* plen) {
    ...
    if (pos < 0) {
        ...
        while (ucount < upos) {
            ssize_t next =
                str_next_ofs(henv->input, henv->input_len, cpos, NULL);
            if (next <= 0)
                return;
            ucount++;
            cpos += next;
...
ic_public void ic_highlight_formatted(ic_highlight_env_t* henv, const char* s,
    ...
    if (attrs != NULL && out != NULL) {
        ...
        if (sbuf_len(out) != len) {
            debug_msg(
                "highlight: formatted string content differs from the original "
                "input:\n  original: %s\n  formatted: %s\n",
                s, fmt);
        }
            ...
            if (c == braces[b]) {
                ...
                if (nesting <= 0) {
                    ...
                    if (open[nesting - 1].close != c && nesting > 1 &&
                        ...
                        attrbuf_update_at(attrs, open[nesting - 1].pos, 1,
                                          error_attr);
                        nesting--;
                    }
                    if (open[nesting - 1].close != c) {
                        ...
                        nesting--;
                        if (i == cursor_pos - 1 ||
                            (open[nesting].at_cursor &&
                             open[nesting].pos != i - 1)) {
                            // highlight matching brace
                            attrbuf_update_at(attrs, open[nesting].pos, 1,
                                              match_attr);
