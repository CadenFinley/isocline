...
static const char* help[] = {
    ...
    "pgdn,"
    "^end",
    "go to the end of the current input", "alt-m", "jump to matching brace",
    "^p", "go back in the history", "^n", "go forward in the history", "^r,^s",
...
static void edit_show_help(ic_env_t* env, editor_t* eb) {
    ...
    for (ssize_t i = 0; help[i] != NULL && help[i + 1] != NULL; i += 2) {
        if (help[i][0] == 0) {
            ...
            bbcode_printf(
                env->bbcode, "  [ic-emphasis]%-13s[/][ansi-lightgray]%s%s[/]\n",
